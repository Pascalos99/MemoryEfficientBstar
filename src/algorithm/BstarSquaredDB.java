...
public class BstarSquaredDB implements SearchAlgorithm {
	...
	public BstarSquaredDB(double effort_ratio, StrategyFunction L1_strategyFunction) {
		this(effort_ratio, L1_strategyFunction, StrategyFunction.PROVEBEST);
	}

	private final double effortRatio;
	...
	public <P extends IGamePosition<P>> SearchResult<?, P>
		...
		adjustNodeCount(1, combineArrays(metrics, L1_metrics));
		StopCondition total_limit = (n, m) -> space_limit.reachedSum(L1_metrics, L2_metrics);
		BstarBasicDB b1 = new BstarBasicDB(total_limit, strategyFunctions[0], effortRatio);
		b1.expectIncorrectBounds(true);
		var L1_result = b1.searchWithTree(L1_tree, time_limit, space_limit, combineArrays(metrics, L1_metrics));
