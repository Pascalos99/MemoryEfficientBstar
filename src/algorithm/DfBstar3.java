...
/**
 * This implementation of df-B* with initial node selection <i>inspired</i> by B* Disprove-Best,
 ...
 */
public class DfBstar3 implements SearchWithTable {
	...
	private <P extends IGamePosition<P>> void dfBstar(DepthFirstNode<P> root, Table TT, Duration time_limit, Limits space_limit, boolean[] foundError, MetricKeeper[] M) {
		...
		while (!foundError[0]
			...
			Thread midThread = new Thread(null, ()->{
				MID(next, tee, tau, TT, timeLimit, space_limit, foundError, strategy, depthLimit, M);
	...
	private <P extends IGamePosition<P>> void MID(DepthFirstNode<P> n, double tee, double tau, Table TT, Duration time_limit, Limits space_limit, boolean[] foundError, boolean[] usePB, long depth, MetricKeeper[] M) {
		Instant start = Instant.now();
		Duration elapsed_time = Duration.ZERO;
		...
		while (!foundError[0] && !foundError[1]
			...
			// Recursive step:
			try {
				MID(next, n_tee, n_tau, TT, time_limit.minus(elapsed_time), space_limit, foundError, usePB, depth-1, M);
